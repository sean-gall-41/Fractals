...
void FractalGUI::doSierpinski() {
    ...
    std::string colorStr = colorChooser->getSelectedItem(); //Implement different triangle colorings
    int color = getColorMap()[colorStr];

    Timer timmy;
    timmy.start();

    drawSierpinskiTriangle(*window, x, y, size, order, color);
...
void FractalGUI::doMengerSponge() {
    if (!hasAllCoords()) {
        setStatusText("Error: You must set an x, y , size, and order first.", /* isError */ true);
        return;
    }
    ...
    int size = sizeField->getValueAsInt();
    int order = orderField->getValueAsInt();
    std::string statusText = "Menger sponge: x=" + integerToString(x)
            + " y=" + integerToString(y)
            + " size=" + integerToString(size)
            + " order=" + integerToString(order);
    setStatusText(statusText + " ...");
    window->setColor(0x0);
    std::string colorStr = colorChooser->getSelectedItem(); //Implement different square colorings
    int color = getColorMap()[colorStr];

    Timer timmyBoy;
    timmyBoy.start();

    drawMengerSponge(*window, x, y, size, order, color);

    long elapsed = timmyBoy.stop();
    statusText += " (" + longToString(elapsed) + "ms)";
    setStatusText(statusText);
}
...
//Below is most definitely used in coloring the Mandelbrot Set
...
void FractalGUI::handleActionEvent(GActionEvent& actionEvent) {
    if (actionEvent.getSource() == sierpinskiButton) {
        ...
        doTree();
    } else if (actionEvent.getSource() == mengerSpongeButton) {
        doMengerSponge();
        ...
        if (!actionEvent.isShiftKeyDown()) {
            ...
            // Before we call mandelbrotSet, we obtain the boolean value
            // determining whether we use the smooth color gradient or not
            bool smoothedGradient = smoothGradient->isChecked();

            mandelbrotSet(*window,minMandelbrotX,mandelbrotIncX,
                          minMandelbrotY,mandelbrotIncY, sizeField->getValueAsInt(),color, smoothedGradient);
...
void FractalGUI::handleMouseEvent(GMouseEvent& mouseEvent) {
    ...
    if (mouseEvent.getEventType() == MOUSE_MOVED) {
        ...
        if (mandelbrotFillMode) {
            ...
            std::string colorStr = colorChooser->getSelectedItem();
            int color = getColorMap()[colorStr];
            // Same situation as in handleActionEvent, the idea being we check
            // the state of the smoothGradient button before the mandelbrotSet function is run
            // this should have the effect of switching between algorithms regardless
            // the level of zoom
            bool smoothedGradient = smoothGradient->isChecked();
            mandelbrotSet(*window,minMandelbrotX,mandelbrotIncX,
                          minMandelbrotY,mandelbrotIncY,sizeField->getValueAsInt(),color, smoothedGradient);
...
void FractalGUI::initialize() {
    ...
    window->setTitle("CS 106B Fractals");

    // create controls
    sierpinskiButton = createButton("Sierpinski Triangle", "triangle-icon.png");
    mengerSpongeButton = createButton("Menger Sponge", "menger_sponge.png");
    ...
    //TODO: Implement smoothGradient Button functionality
    smoothGradient = new GCheckBox("Smooth gradient?");
    smoothGradient->setChecked(true);
    ...
    window->addToRegion(smoothGradient, "North");

    window->addToRegion(sierpinskiButton, "West");
    window->addToRegion(mengerSpongeButton, "West");
    window->addToRegion(treeButton, "West");
    window->addToRegion(mandelbrotButton, "West");
    window->addToRegion(colorChooser, "North"); //TODO: change location of color chooser for generalization?
